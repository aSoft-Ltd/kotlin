...
import tz.co.asoft.internal.klockLazyOrGet
...
data class DateTimeRange(val from: DateTime, val to: DateTime) : Comparable<DateTime> {
    ...
    companion object {
        operator fun invoke(base: Date, from: Time, to: Time): DateTimeRange =
            DateTimeRange(base + from, base + to)
    }
    ...
    private inline fun <T> _intersectionWith(
        that: DateTimeRange,
        rightOpen: Boolean,
        handler: (from: DateTime, to: DateTime, matches: Boolean) -> T
    ): T {
    ...
    fun intersectsWith(that: DateTimeRange, rightOpen: Boolean = true): Boolean =
        _intersectionWith(that, rightOpen) { _, _, matches -> matches }

    fun intersectsOrInContactWith(that: DateTimeRange): Boolean =
        intersectsWith(that, rightOpen = false)
