...
inline class DateTime(
    ...
    operator fun plus(delta: DateTimeSpan): DateTime =
        this.add(delta.totalMonths, delta.totalMilliseconds)
    ...
    operator fun minus(other: DateTime): TimeSpan =
        (this.unixMillisDouble - other.unixMillisDouble).milliseconds
    ...
    fun add(deltaMonths: Int, deltaMilliseconds: Double): DateTime = when {
        ...
        deltaMonths == 0 -> DateTime(this.unixMillis + deltaMilliseconds)
            ...
            DateTime(
                dateToMillisUnchecked(
                    year.year,
                    month,
                    day
                ) + (yearOneMillis % MILLIS_PER_DAY) + deltaMilliseconds
            )
        }
    }
    ...
    fun add(dateSpan: MonthSpan, timeSpan: TimeSpan): DateTime =
        add(dateSpan.totalMonths, timeSpan.milliseconds)
