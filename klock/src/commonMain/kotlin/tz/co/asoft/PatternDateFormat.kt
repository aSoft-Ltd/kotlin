    ...
    internal val regexChunks = chunks.map {
        when (it) {
            ...
            " " -> """(\s+)"""
                ...
                it.startsWith('\'') -> "(" + Regex.escapeReplacement(
                    it.substr(
                        1,
                        it.length - 2
                    )
    ...
    override fun tryParse(str: String, doThrow: Boolean): DateTimeTz? {
        ...
        for ((name, value) in chunks.zip(result.groupValues.drop(1))) {
            ...
            when (name) {
                ...
                "yyy" -> fullYear =
                    value.toInt() + if (value.toInt() < 800) 2000 else 1000 // guessing year...
        ...
        val dateTime =
            DateTime.createAdjusted(fullYear, month, day, hour, minute, second, millisecond)
