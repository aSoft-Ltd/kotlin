...
object ISO8601 {
    ...
    data class BaseIsoDateTimeFormat(val format: String, val twoDigitBaseYear: Int = 1900) :
        DateFormat {
        override fun format(dd: DateTimeTz): String = buildString {
            ...
            while (fmtReader.hasMore) {
                when {
                    ...
                    fmtReader.tryRead("mm,mm") -> append(
                        (time.minutes % 60.0).padded(2, 2).replace('.', ',')
                    )
                    fmtReader.tryRead("mm") -> append(d.minutes.padded(2))
                    fmtReader.tryRead("ss,ss") -> append(
                        (time.seconds % 60).padded(2, 2).replace('.', ',')
                    )
        ...
        private fun tryParse(str: String): DateTimeTz? {
            ...
            while (fmtReader.hasMore) {
                when {
                    ...
                    fmtReader.tryRead("YYYYYY") -> year =
                        reader.tryReadInt(6) ?: return reportParse("YYYYYY")
                    fmtReader.tryRead("YYYY") -> year =
                        reader.tryReadInt(4) ?: return reportParse("YYYY")
                    ...
                    fmtReader.tryRead("MM") -> month =
                        reader.tryReadInt(2) ?: return reportParse("MM")
                    fmtReader.tryRead("DD") -> dayOfMonth =
                        reader.tryReadInt(2) ?: return reportParse("DD")
                    fmtReader.tryRead("DDD") -> dayOfYear =
                        reader.tryReadInt(3) ?: return reportParse("DDD")
                    fmtReader.tryRead("ww") -> weekOfYear =
                        reader.tryReadInt(2) ?: return reportParse("ww")
                    fmtReader.tryRead("D") -> dayOfWeek =
                        reader.tryReadInt(1) ?: return reportParse("D")

                    fmtReader.tryRead("hh,hh") -> hours =
                        reader.tryReadDouble(5) ?: return reportParse("hh,hh")
                    fmtReader.tryRead("hh") -> hours =
                        reader.tryReadDouble(2) ?: return reportParse("hh")
                    fmtReader.tryRead("mm,mm") -> minutes =
                        reader.tryReadDouble(5) ?: return reportParse("mm,mm")
                    fmtReader.tryRead("mm") -> minutes =
                        reader.tryReadDouble(2) ?: return reportParse("mm")
                    fmtReader.tryRead("ss,ss") -> seconds =
                        reader.tryReadDouble(5) ?: return reportParse("ss,ss")
                    fmtReader.tryRead("ss") -> seconds =
                        reader.tryReadDouble(2) ?: return reportParse("ss")
        ...
        fun withTwoDigitBaseYear(twoDigitBaseYear: Int = 1900) =
            BaseIsoDateTimeFormat(format, twoDigitBaseYear)
    }
        ...
        override fun format(dd: DateTimeSpan): String = buildString {
            ...
            while (fmtReader.hasMore) {
                when {
                    ...
                    fmtReader.tryRead("nnM") -> append(if (time) dd.minutes else dd.months).append(
                        'M'
                    )
    ...
    data class IsoDateTimeFormat(val basicFormat: String?, val extendedFormat: String?) :
        DateFormat {
    ...
    val DATE_ALL = listOf(
        DATE_CALENDAR_COMPLETE,
        DATE_CALENDAR_REDUCED0,
        DATE_CALENDAR_REDUCED1,
        DATE_CALENDAR_REDUCED2,
        DATE_CALENDAR_EXPANDED0,
        DATE_CALENDAR_EXPANDED1,
        DATE_CALENDAR_EXPANDED2,
        DATE_CALENDAR_EXPANDED3,
        DATE_ORDINAL_COMPLETE,
        DATE_ORDINAL_EXPANDED,
        DATE_WEEK_COMPLETE,
        DATE_WEEK_REDUCED,
        DATE_WEEK_EXPANDED0,
        DATE_WEEK_EXPANDED1
