...
open class GenerateWebpackHelperTask : DefaultTask() {
    @get:Nested
    val config by lazy {
        project.frontendExtension.bundles().filterIsInstance<WebPackExtension>().single()
    }

    @get:Input
    val webPackConfigFilePath: String by lazy {
        (config.webpackConfigFile?.let { project.file(it) }
            ?: project.buildDir.resolve("webpack.config.js")).absolutePath
    }
    ...
    fun main() {
        val json =
            JsonBuilder(config(project, config, File(webPackConfigFilePath))).toPrettyString()
