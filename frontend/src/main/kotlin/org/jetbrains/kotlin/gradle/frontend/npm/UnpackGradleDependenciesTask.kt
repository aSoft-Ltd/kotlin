...
open class UnpackGradleDependenciesTask : DefaultTask() {
    ...
    fun unpackLibraries() {
        ...
        (compileConfiguration.resolvedConfiguration.resolvedArtifacts +
            ...
            .forEach { artifact ->
                ...
                val existingPackageJson =
                    project.zipTree(artifact.file).firstOrNull { it.name == "package.json" }
                ...
                if (existingPackageJson != null) {
                    ...
                    project.copy { copy ->
                        copy.from(project.zipTree(artifact.file)).into(outDir)
                    }
    ...
    private val moduleNamePattern =
        """\s*//\s*Kotlin\.kotlin_module_metadata\(\s*\d+\s*,\s*("[^"]+")""".toRegex()
