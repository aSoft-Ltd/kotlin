...
import tz.co.asoft.sms.entities.DeliveryReport.Status
import tz.co.asoft.sms.entities.TextMessage
import tz.co.asoft.sms.gateway.ISmsGateway

class SiltechSmsGateway(override val repo: IRepo<TextMessage>, private val apiKey: String) : ISmsGateway {
    private val client = HttpClient { }

    private fun getData(sender: String, receivers: List<Phone>, body: String, type: TextMessage.Type) = mutableListOf<FormPart<Any>>().apply {
    ...
    private suspend fun getRequest(sender: String, receivers: List<Phone>, body: String, type: TextMessage.Type): TextMessage {
        val data = getData(sender, receivers, body, type).joinToString("&") { "${it.key}=${it.value.toString().encodeURLParameter(true)}" }
        val response = client.get<String>("https://siltechtz.com/app/smsapi/index.php?$data")
        val txtMsg = TextMessage(sender, receivers.map { it.value }, type.name, body)
    ...
    override suspend fun send(sender: String, receivers: List<Phone>, body: String, type: TextMessage.Type): TextMessage {
        return getRequest(sender, receivers, body, type)
    }

    private fun DeliveryResponse.toReport() = DeliveryReport(MSISDN, when (DLR) {
