...
import tz.co.asoft.gateway.sms.DeliveryReport
import tz.co.asoft.gateway.sms.TextMessage
import tz.co.asoft.gateway.sms.ISmsGateway
import tz.co.asoft.gateway.sms.TextMessage.Type

class TestGateway(override val repo: IRepo<TextMessage>) :
    ISmsGateway {
    ...
    override suspend fun deliverStatus(uid: String): List<DeliveryReport> {
        return repo.load(uid)?.recipients?.map {
            DeliveryReport(
                it,
